...
<div id="container-wrapper">
  ...
  <!-- if product details exist load info -->
	<?php
	...
	?>
        ...
        <div class="detail_leftbar">
                ...
                <?php if($this->lang->line('user_by') != '') {
                  echo stripslashes($this->lang->line('user_by'));
                } else
                  echo "by"; ?>
                <a href="<?php if ($productDetails->row()->user_id != '0'){
                          echo base_url().'user/'.$productDetails->row()->user_name;
                        } else {
                          echo base_url().'user/administrator';}?>" class="username">
                  <?php if ($productDetails->row()->user_id != '0'){
                    echo $productDetails->row()->full_name;
                  }else {
                    echo 'administrator';}?>
                </a> +
                <?php echo $productDetails->row()->likes;?>
                <?php if($this->lang->line('product_others') != '') {
                  echo stripslashes($this->lang->line('product_others'));
                } else echo "others"; ?>
              </p>
              <br class="hidden">
              ...
              <ul class="detail_thing_info1">
                ...
                ?>
                <!-- Share button -->
						    <li><a href="things/<?php echo $productDetails->row()->id;?>/<?php echo url_title($productDetails->row()->product_name,'-');?>"
                      id="show-someone"
                      class="show"
                      uid="<?php echo $loginCheck;?>"
                      tid="<?php echo $productDetails->row()->id;?>"
                      tname="<?php echo $productDetails->row()->product_name;?>"
                      tuser="<?php if ($productDetails->row()->user_id != '0'){
                              echo $productDetails->row()->full_name;
                            }else {
                              echo 'administrator';}?>"
                      data-timage="<?php //echo base_url();?>images/product/<?php echo $img;?>"
                      price="<?php echo $productDetails->row()->sale_price;?>"
                      reacts="<?php echo $productDetails->row()->likes;?>"
                      username="<?php if ($loginCheck != ''){
                                  if (count($userDetails)>0){
                                    echo $userDetails->row()->user_name;
                                  }
                                }?>"
                      action="buy"
                      require_login="<?php if (count($userDetails)>0){
                                      echo 'false';
                                    }else {
                                      echo 'true';
                                    }?>">
                      <i class="link_icon"></i>
                      <?php if($this->lang->line('header_share') != '') {
                        echo stripslashes($this->lang->line('header_share'));
                      } else
                        echo "Share"; ?>
                    </a>
                </li>
                <!-- Add to List button -->
                      ...
                      class="list"
                      id="show-add-to-list-left">
                      <i class="want_icon"></i>
                      <?php if($this->lang->line('header_add_list') != '') {
                        echo stripslashes($this->lang->line('header_add_list'));
                      } else
                        echo "Add to List";?>
                    </a>
                </li>
                <!-- Feature on my profile button -->
						    <li><a href="#"
                      tid="<?php echo $productDetails->row()->seller_product_id;?>"
                      class="<?php if (count($userDetails)>0){
                              if ($productDetails->row()->seller_product_id == $userDetails->row()->feature_product){
                      ...
                      <i class="feature_icon"></i>
                      <?php if($this->lang->line('product_feature') != '') {
                        echo stripslashes($this->lang->line('product_feature'));
                      } else
                        echo "Feature on my profile"; ?>
                    </a>
                </li>
              <!-- show edit/delete button options if this user is the seller -->
        ...
        <!-- Seller Story -->
          ...
          //check if the user following the product's seller
					$followClass = '';
		        	$followText = 'Follow';
			        if ($loginCheck != ''){
				...
				?>
        <!-- start tabbed panels, TabbedPanels1 is initialized below after all of the content is populated -->
            ...
            <ul class="TabbedPanelsTabGroup">
              ...
              <li tabindex="0" class="TabbedPanelsTab"><?php if($this->lang->line('shipping_policies') != '') { echo stripslashes($this->lang->line('shipping_policies')); } else echo "Shipping & Policies"; ?></li>
                ...
                <div class="TabbedPanelsContentGroup">
                  <!-- start with displaying the description for the first tab "Item Details" -->
                  ...
                  <!-- load content for product feedback tab -->
                  ...
                  <!-- load content for shipping tab -->
                  <div class="TabbedPanelsContent" style="display: none;">
                 <?php echo $productDetails->row()->shipping_policies; ?>
                 <div class="clear"></div>
                  </div>

                  <!-- initialize the TabbedPanels1 -->
          ...
          <!--<button id="btn-viewall-comments" class="toggle">View all 28 comments <i></i></button>
            ...
            //load comments on product
                ...
                //non-active comments
                    ...
                    //if this is the seller
                    //then include approve/delete option
    								if ($loginCheck != '' && $productDetails->row()->user_id > 0 && $loginCheck == $productDetails->row()->user_id){
    		              ?>
          						<li class="comment">
                        <!-- not sure what this milestone thing is yet -->
          							<a class="milestone" id="comment-1866615"></a>
          							<span class="vcard"><a href="<?php echo base_url();?>user/<?php echo $cmtrow->user_name;?>" class="url"><img src="images/users/<?php if($cmtrow->thumbnail!=''){ echo $cmtrow->thumbnail;}else{echo 'user-thumb.png';}?>" alt="" class="photo"><span class="fn nickname"><?php echo ucfirst($cmtrow->user_name);?></span></a></span>
          							<p class="c-text"><?php echo $cmtrow->comments;?></p>
                        ...
                        //if this is the user that commented on the product
                        //then include option to delete the comment
...
<!-- Javascript for submitting comment: start -->
<!-- Insert comment on page when submitted -->
      ...
      <div class="detail_leftbar1">
				...
				<div class="might-fancy">
					...
					<div style="height: 259px;" class="figure-row fancy-suggestions anim might_box">
  					...
  					foreach ($relatedProductsArr as $relatedRow){
  						...
  						$fancyClass = 'fancy';
  						$fancyText = LIKE_BUTTON;
              //see if user has liked product
              //mark as 'fancyd' if they have already liked it
  						if (count($likedProducts)>0 && $likedProducts->num_rows()>0){
  							foreach ($likedProducts->result() as $likeProRow){
  					 ...
  					 ?>
             <!-- thumbnail picture for products -->
        ...
        <div class="detail_leftbar1">
          <h3 id="recently-fancied-by" class="detail_link_list"><?php if($this->lang->line('recently') != '') { echo stripslashes($this->lang->line('recently')); } else echo "Recently"; ?> <?php echo LIKED_BUTTON;?> <?php if($this->lang->line('user_by') != '') { echo stripslashes($this->lang->line('user_by')); } else echo "by"; ?>...</h3>
            ...
            //get recent likes but filter out this user
              ...
              ?>
              <!-- each figure-row is a user and products that they liked -->
      ...
      <!-- aside with pricing/quantity and checkout -->
          ...
          <div class="detail_sidebar">
            ...
            <div class="thing-description">
              ...
              ?>
            <!-- 'more' button triggers popup of product see dlg_detail in thing_page.js -->
            ...
            <!-- display quick shipping if product is eligible to ship immediately -->
            <div class="quick-shipping" <?php if($productDetails->row()->ship_immediate == 'true'){?>style="display: block;"<?php }?>>
              <span class="icon truck"></span>
                <?php if($this->lang->line('header_immed_ship') != '') {
                  echo stripslashes($this->lang->line('header_immed_ship'));
                } else echo "Immediate Shipping";
                ?>
                <span class="tooltip">
                  <i class="icon"></i>
                  <small>
                    <?php if($this->lang->line('header_ship_within') != '') {
                      echo stripslashes($this->lang->line('header_ship_within'));
                      } else echo "Ships within 1-3 business days";
                    ?>
                    <b></b>
                  </small>
                </span>
            ...
            <div class="option-area detail_option1">
              <div class="detail_option">
                ...
                <?php
                  ...
                  //load them so that the user can choose
                ...
                ?>
                  ...
                  <select name="attr_name_id" id="attr_name_id" class="option  selectBox" style="border:1px solid #D1D3D9;width: 190px;" onchange="ajaxCartAttributeChange(this.value,'<?php echo $productDetails->row()->id; ?>');" >
                    ...
                    <?php
                      foreach($PrdAttrVal->result_array() as $Prdattrvals ){
                    ?>
                ...
                <!-- hidden inputs with product details -->
                ...
                <!-- ajax_add_cart defined in validation.js -->
                <input
                  type="button"
                  class="greencart add_to_cart" <?php if ($loginCheck==''){echo 'require_login="true"';}?>
                  name="addtocart"
                  value="<?php if($this->lang->line('header_add_cart') != '') {
                            echo stripslashes($this->lang->line('header_add_cart'));
                          } else echo "Add to Cart";
                        ?>"
                  onclick="ajax_add_cart('<?php echo $PrdAttrVal->num_rows; ?>');"
                />
              </div>
            </div>
          </div>
            ...
            <ul class="detail_thinginfo">
              <?php
                //load the values for the actions
 ...
 -->
                <ul>
        					...
        					if ($seller_product_details->num_rows()>0){
        					  ...
        					  ?>
                    <li>
                      <a href="things/<?php echo $seller_product_details_row->id;?>/<?php echo url_title($seller_product_details_row->product_name,'-');?>" class="figure-img">
                    ...
                    ?>
                    <li>
                      <a href="user/<?php echo $productDetails->row()->user_name;?>/things/<?php echo $seller_affiliate_products_row->seller_product_id;?>/<?php echo url_title($seller_affiliate_products_row->product_name,'-');?>" class="figure-img">
  						          <span style="background-image: url(<?php echo base_url();?>images/product/<?php echo $img;?>);"></span>
                      </a>
                    </li>
                    <?php
    ...
    <!-- if product details are not available -->
		<?php
    }else {
    ?>
...
<!-- product_detail.php loading the javascript -->
...
/**
 * Product Quantity/Price functions
 */
...
//called in popup/product.php when input quantity box updated
