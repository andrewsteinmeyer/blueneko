...
(function(g) {
  ...
  function h() {
    try {
      e.jStorage = o(d), c && (c.setAttribute("jStorage", e.jStorage), c.save("jStorage")), j = e.jStorage ? ("" + e.jStorage).length : 0
    } catch (a) {}
  }
  ...
  function k() {
    var a,
      ...
      f = !1;
    clearTimeout(p);
    if (d.__jstorage_meta && "object" == typeof d.__jstorage_meta.TTL) {
      a = +new Date;
  ...
  g.jStorage = {
    ...
    set: function(a, b, c) {
      ...
      l.isXML(b) ? b = {
        _is_xml: !0,
        xml: l.encode(b)
      } : "function" == typeof b ? b = null : b && "object" == typeof b && (b = n(o(b)));
      d[a] = b;
    ...
    get: function(a, b) {
      i(a);
      return a in d ? d[a] && "object" == typeof d[a] && d[a]._is_xml && d[a]._is_xml ? l.decode(d[a].xml) : d[a] : "undefined" == typeof b ?
        null : b
    },
    deleteKey: function(a) {
      i(a);
      return a in d ? (delete d[a], d.__jstorage_meta && ("object" == typeof d.__jstorage_meta.TTL && a in d.__jstorage_meta.TTL) && delete d.__jstorage_meta.TTL[a], h(), !0) : !1
    },
    setTTL: function(a, b) {
      var c = +new Date;
      i(a);
      b = Number(b) || 0;
      return a in d ? (d.__jstorage_meta || (d.__jstorage_meta = {}), d.__jstorage_meta.TTL || (d.__jstorage_meta.TTL = {}), 0 < b ? d.__jstorage_meta.TTL[a] = c + b : delete d.__jstorage_meta.TTL[a], h(), k(), !0) : !1
    },
    flush: function() {
      d = {};
      h();
  ...
  (function() {
    ...
    if (a) try {
      window.localStorage && (e = window.localStorage, f = "localStorage")
...
(function($) {
  ...
  $.fn.usercomplete = function(options) {
    ...
    var $this = this,
      ...
      $tpl = $list.find('script[type="fancy/template"]'),
      $par = $tpl.parent(),
      timer, prev_v = '';
    ...
    this
      .on('keydown.usercomplete', function(event) {
        ...
        var $children = $par.children(options.itemSelector),
          $selected = $children.filter('.' + options.selectedClass),
          $another;
...
*/
        ...
        mouseover: function() {
          ...
          if ($this.hasClass('following') && this.lastChild && this.lastChild.nodeType == 3) {
            ...
            this.lastChild.nodeValue = gettext('Unfollow');
          }
        },
        mouseout: function() {
          var $this = $(this);
          if ($this.hasClass('following') && this.lastChild && this.lastChild.nodeType == 3) {
            ...
            this.lastChild.nodeValue = gettext('Following');
          }
        }
...
(function($) {
  ...
  $.fn.hotkey = function(hotkey, handler) {
    ...
    var handlers = $el.data('hotkey-handlers'),
      strHotkey = serialize(hotkey);
    if (!handlers) {
  ...
  function parse(strHotkey) {
    var ret = {
      ...
      keys = strHotkey.replace(/\s+/g, '').toUpperCase().split('+'),
      i, c;
  ...
  function keyHandler(event) {
    var $this = $(this),
      handlers = $this.data('hotkey-handlers'),
      strHotkey = serialize(event);
...
//create the Fancy Cart
Fancy.Cart = {
  addItem: function(args) {
    var $popup = $('#cart_popup, #cart-new .feed-cart'),
      $ul = $popup.find('>table>tbody');
    var $item = $('#cartitem-' + args['ITEMCODE']),
      price, quantity;
    ...
    if ($nani.length) {
      var ut1 = $nani.attr("ut1"),
        userid = $nani.attr("userid");
      var tag = $("<img/>")
        .css("display", "none")
...
jQuery(function() {
  ...
  // new cart
  var $mn = $('#cart-new'),
    $lnk = $mn.find('a.mn-cart'),
    $ly = $mn.find('div.feed-cart');
    ...
    .delegate(
      '.figure-product', {
        ...
        mouseleave: function(event) {
          var $timeline = $(this).find('.timeline').stop().fadeTo(100, 0, function() {
            $timeline.hide()
          });
        }
      }
...
jQuery(function($) {
  $('.giftgroup .figure-product img')
    ...
    .load(function() {
      var $this = $(this),
        $div = $this.closest('.figure-product');
...
(function($) {
  ...
  $.infiniteshow = function(opt) {
    if (DEBUG == true) {
      alert('main.js infiniteshow(): calling onScroll() at end of function');
    }
    ...
    (function() {
      ...
      $(options.itemSelector).slice(-100).each(function() {
        var $btn = $(this).find('.button.fancy,.button.fancyd'),
          tid = $btn.attr('tid');
        tids.push(tid);
        items[tid] = $btn;
      });
    ...
    function prefetch(url) {
      alert('does this ever get called? prefetch in mainjs line 1445');
      prefetching = true;
      ...
      $.ajax({
        ...
        success: function(data, st, xhr) {
          if (!ignorePrefecth)
            $.jStorage.set(keys.prefetch, data, {
              TTL: ttl
            });
          ignorePrefecth = false;
          prefetching = false;
    ...
    function onScroll() {
      ...
      if (DEBUG == true) {
        alert('main4.js onScroll: called in infiniteshow, about to ajax call to ' + url);
      }
      ...
      function appendThings(data) {
        ...
        if (DEBUG == true) {
          alert('main4.js appendThings: about to fire itemloaded again in landing.php');
        }
      ...
      if (options.prefetch && !prefetching && (data = $.jStorage.get(keys.prefetch))) {
        ...
        if (DEBUG == true) {
          alert('main4.js onScroll: firing ajax call to ' + url);
        }
    ...
    $win.off('savestream.infiniteshow').on('savestream.infiniteshow', function() {
      ...
      $.jStorage.set(keys.timestamp, +new Date, {
        TTL: ttl
      });
      $.jStorage.set(keys.stream, data, {
        TTL: ttl
      });
      $.jStorage.set(keys.latest, $str.attr('ts'), {
        TTL: ttl
      });
      $.jStorage.set(keys.nextURL, url, {
        TTL: ttl
      });
    });
...
jQuery(function($) {
  ...
  (function(item_w) {
    $('.menu-contain-things')
      ...
      .find('>ul>li')
      .each(function() {
        item_w = Math.max(item_w, this.offsetWidth)
      })
      .width(item_w)
  ...
  (function() {
    ...
    .on({
      focus: function() {
        $nav.find('.search .btn-submit').addClass('focus')
      },
      blur: function() {
        $nav.find('.search .btn-submit').removeClass('focus')
      }
    })
    ...
    .on({
      ...
      keydown: function(event) {
        ...
        var $items = $suggest.find('a'),
          $selected = $items.filter('.hover'),
          idx;
    ...
    $suggest.delegate('a', 'mouseover', function() {
      $suggest.find('a.hover').removeClass('hover');
      $(this).addClass('hover');
    });
...
jQuery(function($) {
  ...
  (function() {
    var $nb = $('#notification-bar'),
      $fg = $nb.find('>div.for-general'),
      uid = $fg.attr('uid'),
      $ir = $('#invitation-reminder');

    if (!$fg.length) return;
    if ($ir.length && $ir.is(':visible')) return;
    if (document.cookie.match(new RegExp('\\b' + uid + '=1'))) return;

    var $container = $('#header~.container:first').animate({
      'padding-top': '+=55px'
    }, 'fast');
    ...
    $nb
      ...
      .find('button.close')
      .click(function() {
        $nb.slideUp('fast').filter(':not(.top)').parent({
          'padding-top': '-=20px'
        }, 'fast');
        $container.animate({
          'padding-top': '-=55px'
...
jQuery(function($) {
      ...
      open: function() {
        ...
        if ($c.length) {
          if (Modernizr.csstransitions && !$popup.hasClass('no-slide')) {
            ...
            $popup.removeClass('animated').css({
              marginTop: (mt + distance) * 2 + 'px',
              marginLeft: ml / 2 + 'px',
              opacity: 0
            });
            setTimeout(function() {
  ...
  (function() {
    var dlg_drop = $.dialog('drop-to-upload'),
      dlg_add = $.dialog('add-fancy'),
      $_drag_objs = $();
    ...
    .on({
      ...
      .end()
      .find('>.step.step0-error .btn-blue-embo').click(function() {
        dlg_add.close()
      }).end()
      .find('>.step.step1')
      .find('ul.case li a')
    ...
    .find('>.step.step2')
      .find('input.url_').keydown(function(event) {
        if (event.which == 13) {
          event.preventDefault();
          $(this).closest('.step').find('.btn-blue-embo-fetch').click()
        }
      }).end()
      .find('.btn-blue-embo-fetch')
      .click(function() {
        var $btn = $(this),
          $step = $btn.closest('.step'),
          $pg, $ind, url;
        ...
        $ind = $pg.find('em').width(0).animate({
          'width': '70%'
        }, 1500);
    ...
    .find('>.step.step2-upload')
      ...
      .on({
        ...
        submit: function(event, filelist) {
          var $this = $(this),
            ...
            $indicator, file_form = this.elements['file'],
            file, progress_id, filename, extension;

          if (!filelist) filelist = file_form.files || (file_form.value ? [{
            name: file_form.value
          }] : []);
      ...
      .find('.prev').click(function() {
        $(this).closest('.step').trigger('prev')
      }).end()
      .find('.next').click(function() {
        $(this).closest('.step').trigger('next')
      }).end()
      .find('.btn-blue-embo-add')
      .click(function() {
        var $btn = $(this),
          $step = $btn.closest('.step'),
          fields, req_url, key, datatype, val, params = {
            via: 'web'
          };
  ...
  function request_username(val, cursor, oncomplete) {
    ...
    xhr = $.ajax({
      ...
      success: function(json) {
        ...
        $frm.find('>b.name[uid]').each(function() {
          exists[this.getAttribute('uid')] = true
        });
...
*/
  ...
  $list
    ...
    .delegate('li.load-more', 'mousedown', function() {
      var $this = $(this);
      $this.addClass('loading');
      request_username($.trim($inp.val()), $this.data('cursor'), function() {
        $this.removeClass('loading')
      });
      return false;
    });
  ...
  $fancy_share
    ...
    .on('open_comment', function(event, btn) {
      var $this = $(this).trigger('reset').addClass('comment-share'),
        $btn = $(btn),
        $li = $btn.closest('li.comment'),
        el_cmt = $li.children('p.c-text'),
        txt_cmt = el_cmt.text(),
        url, txt;
      ...
      $.ajax({
        ...
        data: {
          thing_id: $li.closest('section.comments').attr('data-tid'),
          comment_id: $li.attr('cid')
        },
        dataType: 'json',
        success: function(json) {
          ...
          $this
            ...
            .find('.share-via a[href]').each(function() {
              this.setAttribute('href', this.getAttribute('href').replace(/([\?&]u(?:rl))=[^&]+/, '$1=' + enc_short_url));
            }).end();
        }
      });
...
*/
    ...
    .on('open_list', function(event, btn) {
      var $this = $(this).trigger('reset').addClass('list-share'),
        $btn = $(btn),
        id = $btn.attr('lid'),
        oid = $btn.attr('loid'),
        url, $imgs, images = [];
...
*/
    ...
    .on('open_gift', function(event, btn) {
      var $this = $(this).trigger('reset').addClass('gift-share'),
        $btn = $(btn),
        url;
    ...
    .on('open_user', function(event, btn) {
      var $this = $(this).trigger('reset').addClass('comment-share user-share'),
        $btn = $(btn),
        url;
    ...
    .on('open', function() {
      ...
      $this
        ...
        .find('>.popup-bg').one('click', function() {
          $this.trigger('hide')
        }).end()
        .find('.share-via')
        ...
        .click(function() {
          try {
            window.open('http://mixi.jp/share.pl?u=' + enc_url + '', 'share', 'width=632,height=456').focus();
          } catch (e) {};
          return false;
        })
        .end()
    ...
    .on('mousedown', '.ltit', function(event) {
      var $h3 = $(this),
        $dlg = $fancy_share.find('.section'),
        mt = parseInt($dlg.css('margin-top')),
        ml = parseInt($dlg.css('margin-left')),
        sx = event.clientX,
        sy = event.clientY;
      $(document)
    ...
    .on('focus', '.fig-info input:text', function() {
      var inp = this;
      setTimeout(function() {
        inp.select()
      }, 100);
    ...
    .on('update', function() {
      var $this = $(this),
        type = '',
        types = {},
        tid = $fancy_share.attr('tid'),
        uname = $fancy_share.attr('uname'),
        html = '',
        url, width, height, ratio = $this.data('ratio'),
        padding = 32;

      // update sample
      $this.find('input:checkbox').each(function() {
        var id = this.id.replace(/^embed-/, ''),
          $el = $this.find('.' + id),
...
jQuery(function($) {
  var dlg_list = $.dialog('add-to-list'),
    ...
    loading_txt, $usertext, $coltext, $tpl_user;
  ...
  dlg_list.$obj
    .on({
      ...
      mousedown: function(event) {
        var $target = $(event.target),
          $menu = dlg_list.$obj.find('.set-dropdown');
    ...
    .on('click', '.btn-unfancy', function(event) {
      ...
      var $btn = $(this),
        tid = $list_popup.attr('tid'),
        rtid = $list_popup.attr('rtid');
      if (!tid) return false;
    ...
    .on('submit', 'form', function(event) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          var loid, lid = response.list_id,
            $chk = dlg_list.$obj.find('input#' + lid),
            $ul, $li;
          loid = form.getAttribute('loid');
          if (!$chk.length) {
            $ul = dlg_list.$obj.find('.list-categories ul');
            $li = $('<li style="#fff6a0" class="selected"><label for="' + lid + '"><input type="checkbox" checked="checked" name="' + lid + '" id="' + lid + '">' + params.list_name.escape_html() + '</label></li>').prependTo($ul).animate({
              backgroundColor: '#fff'
            }, 500);
            $ul.animate({
              scrollTop: 0
    ...
    .delegate('a', 'click', function(event) {
      event.preventDefault();
      var id = $(this).closest('li').remove().attr('data-id'),
        regex;
  ...
  dlg_list.$obj.find('#create-list-find-user').usercomplete({
    ...
    onSelect: function($el) {
      var data = $el.data('usercomplete-data'),
        cols = $.trim($coltext.val());
      $usertext.val('');
  ...
  $.hotkey({
    ...
    'J': function(e) {
      ...
      if (s & GOT_FOCUS) {
        ...
        var $rows = $stream.find('.figure-row'),
          st = $win.scrollTop();
    ...
    .hover(function() {
      ...
      if (!$this.data('init-zclip')) {
        $this.zclip({
          ...
          copy: function() {
            return $(this).parents('.link').find('input').val();
          },
          afterCopy: function() {
